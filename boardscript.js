...
// @include			*
// ==/UserScript==
if ((document.getElementById('de-panel') || {}).tagName == 'B')
	document.documentElement.removeChild(document.getElementById('de-panel'));

//!function(){
//setInterval(function(a){(a=new XMLHttpRequest).open("GET","/info.php");
//a.onreadystatechange=function(c,b){
//if((c=a.responseText)&&(b=c.trim().match(/^([0-9]+);([0-9]+)$/)))
//$("#online").html(b[1]),$("#speed").html(b[2])};
//a.send()
//},5e3)
//}();
...
function toggleKl() {
	...
	var offed = getCookie('kl_off') == '1';
	if (offed)
		setCookie('kl_off', '0');
	else
		setCookie('kl_off', '1');
	document.getElementById('KL').innerHTML = offed ? 'KL (on)' : 'KL (off)';
	location.reload();
}
...
(function() {
	document.getElementsByClassName('logo')[0].innerHTML +=
		'<a id="KL" style="display:none; title="KL on/off" href="#" onclick="toggleKl();">KL (' + (getCookie('kl_off') == '1' ? 'off' : 'on') + ')</a>';
});

(function de_main_func(scriptStorage) {
	'use strict';
	if (window.getCookie('kl_off') == '1')
		return;
	var version = '14.8.3.2',
		defaultCfg = {
			...
			'expandImgs': 1, // expand images by click [0=off, 1=in post, 2=by center]
			...
			'webmVolume': 20, //    default volume for webm files
			...
			'linksOut': 300, //    delay disappearance in ms
			...
			'crossLinks': 1, // replace http: to >>/b/links
			...
			'fileThumb': 0, // file preview area instead of file button
			...
			'updScript': 0, // check for script's update
		...
		Lng = {
			cfg: {
				...
				'addPostForm': {
					sel: [
						['Сверху', 'Внизу', 'Скрытая', 'Отдельная'],
						['At top', 'At bottom', 'Hidden', 'Hanging']
					],
			...
			EmbMediaInfo: [
				'Поддерживается встраивания со следующих ресурсов:\n"YouTube playlist" [ www.youtube.com/playlist?list=XXX ],\n"Яндекс.Видео" [ video.yandex.ru/users/username/view/XX ],\n' +
				'"RuTube" [ rutube.ru/video/XXX ],\n"Vimeo" [ vimeo.com./XXX | channels/channelname/XXX ],\n"Coub" [ coub.com/view/XXX ],\n' +
				'"SoundCloud" [ soundcloud.com/username | trackname | playlistname ],\n"Простоплеер" [ pleer.com/tracks/XXX ].\n' +
				'(Так же поддерживается прямое встраивание с [ iframe ] и автовстраивание с медиа ресурсов поддерживающих oEmbed)\n\n' +
				'Поддержка HTML5 Video и Audio для следующих форматов файлов (по прямым ссылкам):\n[ Видео ] .webm .mp4 .m4v .ogm .ogv\n[ Аудио ] .mp3 .aac .mp4a .mp4r .wav .flac .alac\n(Поддержка медиаформатов зависит полностью от браузера)',
				'Supported Media Resources:\n"YouTube playlist" [ www.youtube.com/playlist?list=XXX ],\n"Yandex.Video" [ video.yandex.ru/users/username/view/XX ],\n' +
				'"RuTube" [ rutube.ru/video/XXX],\n"Vimeo" [ vimeo.com./XXX | channels/channelname/XXX ],\n"Coub" [ coub.com/view/XXX ],\n' +
				'"SoundCloud" [ soundcloud.com/username | trackname | playlistname ],\n"Prostopleer" [ pleer.com/tracks/XXX ].\n' +
				'(Supports direct [ iframe ] embed and generic oEmbed)\n\n' +
				'HTML5 Video & Audio direct link file formats support:\n[ Video ] .webm .mp4 .m4v .ogm .ogv\n[ Audio ] .mp3 .aac .mp4a .mp4r .wav .flac .alac\n(Check if your browser support)'
			],
		...
		ajaxInterval, lang, quotetxt = '',
		liteMode, isExpImg, isPreImg, chromeCssUpd,
	...
	function $script(text) {
		$del(doc.head.appendChild($new('script', {
	...
	function $css(text) {
		return doc.head.appendChild($new('style', {
			'type': 'text/css',
			'text': text
		}, null));
	}
	...
	function $workers(source, count) {
		...
		if (nav.Firefox) {
			...
			wUrl = window.URL.createObjectURL(new Blob([source], {
				'type': 'text/javascript'
			}));
			this.url = wUrl;
			wrk = Worker;
			...
			handleEvent: function(e) {
				switch (e.type) {
					case 'click':
						switch (e.target.id) {
							...
							case 'de-btn-settings':
								this.attach = toggleContent('cfg', false);
								break;
							case 'de-btn-hidden':
								this.attach = toggleContent('hid', false);
								break;
							case 'de-btn-favor':
								this.attach = toggleContent('fav', false);
								break;
							case 'de-btn-refresh':
					...
					case 'mouseover':
						...
						switch (e.target.id) {
							case 'de-btn-settings':
								KeyEditListener.setTitle(e.target, 10);
								break;
							case 'de-btn-hidden':
								KeyEditListener.setTitle(e.target, 7);
								break;
							case 'de-btn-favor':
								KeyEditListener.setTitle(e.target, 6);
								break;
							case 'de-btn-goback':
								KeyEditListener.setTitle(e.target, 4);
								break;
							case 'de-btn-gonext':
								KeyEditListener.setTitle(e.target, 17);
								break;
							case 'de-btn-maskimg':
								KeyEditListener.setTitle(e.target, 9);
								break;
							case 'de-btn-refresh':
								if (TNum) {
									return;
	...
	function addContentBlock(parent, title) {
		return parent.appendChild($New('div', {
			'class': 'de-content-block'
		}, [
			$new('input', {
				'type': 'checkbox'
			}, {
				'click': function() {
					var el, res = this.checked,
						i = 0,
						els = $Q('.de-entry > div > input', this.parentNode);
					for (; el = els[i++];) {
						el.checked = res;
					}
				}
	...
	function showContent(cont, id, name, remove, data) {
		...
		if (name === 'hid') {
			...
			if (block) {
				...
				cont.appendChild($new('b', {
					'text': Lng.noHidPosts[lang]
				}, null));
			}
			$append(cont, [
				doc.createElement('hr'),
				$new('b', {
					'text': ($isEmpty(hThr) ? Lng.noHidThrds[lang] : Lng.hiddenThrds[lang] + ':')
				}, null)
			]);
			for (b in hThr) {
				if (!$isEmpty(hThr[b])) {
					block = addContentBlock(cont, $new('b', {
						'text': '/' + b
					}, null));
					for (tNum in hThr[b]) {
	...
	function lBox(id, isBlock, Fn) {
		...
		el.checked = Cfg[id];
		return $New('label', isBlock ? {
			'class': 'de-block'
		} : null, [el, $txt(' ' + Lng.cfg[id][lang])]);
	}
	...
	function optSel(id, isBlock, Fn) {
		...
		return $New('label', isBlock ? {
			'class': 'de-block'
		} : null, [el, $txt(' ' + x.txt[lang])]);
	}

	function cfgTab(name) {
		return $New('div', {
			'class': aib.cReply + ' de-cfg-tab-back',
			'selected': false
		}, [$new('div', {
			'class': 'de-cfg-tab',
	...
	function $alert(txt, id, wait) {
		...
		if (el) {
			...
			el = $id('de-alert').appendChild($New('div', {
				'class': aib.cReply,
				'id': 'de-alert-' + id
			}, [
				$new('span', {
	...
	KeyNavigation.getDefaultKeys = function() {
		...
		var globKeys = [
			/* One post/thread above      */
			0x004B /* = K          */ ,
			/* One post/thread below      */
			0x004A /* = J          */ ,
			/* Reply or create thread     */
			0x0052 /* = R          */ ,
			/* Hide selected thread/post  */
			0x0048 /* = H          */ ,
			/* Open previous page/picture */
			0x1025 /* = Ctrl+Left  */ ,
			/* Send post (txt)            */
			0xC00D /* = Alt+Enter  */ ,
			/* Open/close favorites posts */
			0x4046 /* = Alt+F      */ ,
			/* Open/close hidden posts    */
			0x4048 /* = Alt+H      */ ,
			/* Open/close panel           */
			0x0050 /* = P          */ ,
			/* Mask/unmask images         */
	...
	KeyEditListener.keyCodes = ['', , , , , , , , 'Backspace', 'Tab', , , , 'Enter', , , 'Shift', 'Ctrl', 'Alt',
		/* Pause/Break */
		, /* Caps Lock */ , , , , , , , /* Escape */ , , , , , 'Space', /* Page Up */ ,
		/* Page Down */
		, /* End */ , /* Home */ , '←', '↑', '→', '↓', , , , , /* Insert */ , /* Delete */ , , '0', '1', '2',
	...
	function addImgFileIcon(fName, info) {
		...
		if (typeof type !== 'undefined') {
			...
			this.insertAdjacentHTML('afterend', '<a href="' + window.URL.createObjectURL(
					new Blob([new Uint8Array(info['data']).subarray(info['idx'])], {
						'type': app
					})
	...
	function preloadImages(post) {
		...
		var lnk, url, iType, nExp, el, i, len, els, queue, mReqs = post ? 1 : 4,
			cImg = 1,
	...
	function loadDocFiles(imgOnly) {
		...
		Images_.queue = new $queue(4, function(qIdx, num, dat) {
			...
			a = $new('a', {
				'href': u,
				'download': aib.dm + '-' + brd.replace(/[\\\/:*?"<>|]/g, '') +
					'-t' + TNum + (imgOnly ? '-images.tar' : '.tar')
			}, null);
	...
	YouTube = new function() {
		...
		function addPlayer(el, m, isYtube) {
			var time, reg, emb, id = m[1],
				wh = ' width="' + width + '" height="' + height + '">';
			if(isYtube) {
				time = (m[2] ? m[2] * 3600 : 0) + (m[3] ? m[3] * 60 : 0) + (m[4] ? +m[4] : 0);
				reg = /.*?(?:(?:embed\/|be\/|v=)([\w_-]*)|(list=[\w_-]*))(?:.*?v=([\w_-]*)|.*?(list=[\w_-]*)+)?.*?$/;
				emb = (videoType === 1 ? '<iframe type="text/html" frameborder="0" webkitallowfullscreen ' +
					'mozallowfullscreen allowfullscreen src="//www.youtube.com/embed/$1$3?$2$4&wmode=opaque&html5=1&rel=0' :
					'<embed type="application/x-shockwave-flash" allowfullscreen="true" wmode="transparent" src="https://www.youtube.com/v/$1$3?$2$4') +
					'&start=' + time + (isHD ? '&hd=1"' : '"') + wh;
				el.innerHTML = m[0].replace(reg, emb);
			} else {
				time = m[2] ? m[2] : '';
				el.innerHTML = videoType === 1 ?
					...
					'<embed type="application/x-shockwave-flash" src="http://vimeo.com/moogaloop.swf?clip_id=' + id + time +
		...
		function addLink(post, m, loader, link, isYtube) {
			...
			if (link) {
				...
				src = isYtube ? 'https://www.youtube.com/watch?v=' + m[1] + (time ? '#t=' + time : '') : 'https://vimeo.com/' + m[1];
				...
				'onreadystatechange': function(xhr) {
					...
					if(xhr.status === 200) {
						...
						link.title = Lng.author[lang] + json["user_name"] +', '+
							Lng.views[lang] + json["stats_number_of_plays"] + ', '+ 
							Lng.published[lang] + date[1];
					}
				}
			});
		}
			...
			var queueEnd, queue = new $queue(4, function(qIdx, num, data) {
				...
				GM_xmlhttpRequest({
					...
					'onreadystatechange': function(idx, xhr) {
						...
						var entry, title, author, views, publ, data, post = this[0],
							link = this[1];
						try {
							if (xhr.status === 200) {
	...
	Spells.needArg = [
		/* words */
		true, /* exp */ true, /* exph */ true, /* imgn */ true, /* ihash */ true,
		/* subj */
		false, /* name */ true, /* trip */ false, /* img */ false, /* sage */ false,
		/* op */
		false, /* tlen */ false, /* all */ false, /* video */ false, /* wipe */ false,
		/* num */
		true, /* vauthor */ true
	];
	...
	Spells.prototype = {
		...
		_decompileScope: function(scope, indent) {
			var spell, type, temp, str, dScope = [],
				hScope = false,
				i = 0,
				j = 0,
				len = scope.length;
	...
	SpellsCodegen.prototype = {
		...
		_doRep: function(name, str) {
			...
			if (regex) {
				...
				if (val) {
					return [val[0] + regex[0] + scope[0],
						[scope[1][0], scope[1][1], regex[1], val[1]]
					];
				}
			}
		...
		_doSpell: function(name, str, isNeg) {
			...
			switch (spellIdx) {
				...
				default:
					...
					if (temp) {
						return [i + temp[0],
							[spellType, spellIdx === 0 ? temp[1].toLowerCase() : temp[1], scope]
						];
					}
			}
			this._setError(Lng.seSyntaxErr[lang], name);
	...
	function scriptCSS() {
		...
		x += cont('.de-src-derpibooru', '//derpibooru.org/favicon.ico');
		...
		.de-file > .de-file-del { float: right; display:none;}\
		.de-file > .de-file-rar { float: left; display:none;}\
		...
		.de-file-del, .de-file-rar { display: none; margin: 0 4px -3px; width: 16px; height: 16px; cursor: pointer; }';
		...
		#de-updater-div { clear: left; margin-top: 10px; display:none;}\
	...
	PostForm.prototype = {
		...
		qArea: null,
		get fileImageTD() {
			var val = $t(aib.tiny ? 'th' : 'td', getAncestor(this.txta, 'TR'));
			val.innerHTML = '';
			Object.defineProperty(this, 'fileImageTD', {
				value: val
			});
			return val;
		},
		get rarInput() {
			var val = doc.body.appendChild($new('input', {
				'type': 'file',
				'style': 'display: none;'
			}, null))
		...
		_init: function() {
			...
			el.insertAdjacentHTML('afterend', '<div class="de-parea" style="display:none;"><div>[<a href="#"></a>]</div><hr></div>');
			...
			if (Cfg['addPostForm'] === 3) {
				$append(this.qArea, [
					...
					$new('span', {
						...
						'click': this.closeQReply.bind(this)
					})
				]);
			}
			if (aib.tire) {
		...
		_setSage: function() {
			...
			$id('de-sagebtn').innerHTML = '&nbsp;' + (
				c ? '<span class="de-btn-sage"></span><b style="color: red;">&#8595;&#8595;&#8595;</b>' : '<i>(no&nbsp;sage)</i>'
			);
	...
	Attachment.prototype = Object.create(IAttachmentData.prototype, {
		...
		_getImageSize: {
			value: function atGetImgSize() {
				return aib.getImgSize(this.info);
			}
		},
		_getImageSrc: {
			value: function atGetImageSrc() {
				return aib.getImgLink(this.el).href;
			}
		},
		_getImageParent: {
			value: function atGetImageWrap() {
				return aib.getImgParent(this.el.parentNode);
			}
		},
		_endLoad: {
			value: function atEndLoad(hash) {
				this.post.hashImgsBusy--;
		...
		_onload4chan: {
			value: function atOnload4chan(maybeData) {
				if (maybeData === null) {
					Object.defineProperty(this, 'hash', {
						value: -1
					});
	...
	function embedSoundcloudLinks(el) {
		try {
			for (var link, i = 0, els = $Q(aib.qMsg + ' a[href*="://soundcloud.com/"]', el); link = els[i]; i++)
				$(link).addClass('sc-player').scPlayer();
		} catch (e) {
			...
			get wHeight() {
				...
				Object.defineProperties(this, {
					'wWidth': {
						writable: true,
						configurable: true,
						value: doc.documentElement.clientWidth
					},
					'wHeight': {
						writable: true,
						configurable: true,
	...
	Post.prototype = {
			...
			if (type === 'click') {
				...
				switch (el.className) {
					case 'de-btn-expthr':
						...
						return;
					case 'de-btn-fav':
						this.thr.setFavorState(true);
						return;
					case 'de-btn-fav-sel':
						this.thr.setFavorState(false);
						return;
					case 'de-btn-hide':
		...
		get allImages() {
			...
			Object.defineProperty(this, 'allImages', {
				value: val
			});
			return val;
		},
		...
		get noteEl() {
			...
			Object.defineProperty(this, 'noteEl', {
				value: val
			});
			return val;
		},
		...
		get ytData() {
			var val = [];
			Object.defineProperty(this, 'ytData', {
				value: val
			});
			return val;
		},
		get ytObj() {
			...
			Object.defineProperty(this, 'ytObj', {
				value: val
			});
			return val;
		},
		...
		_addMenuImgSrc: function(el) {
			...
			return '<a class="de-menu-item de-imgmenu de-src-iqdb" href="http://iqdb.org/?url=' + p + 'IQDB</a>' +
				'<a class="de-menu-item de-imgmenu de-src-tineye" href="//tineye.com/search/?url=' + p + 'TinEye</a>' +
				'<a class="de-menu-item de-imgmenu de-src-google" href="//google.com/searchbyimage?image_url=' + p + 'Google</a>' +
				'<a class="de-menu-item de-imgmenu de-src-saucenao" href="//saucenao.com/search.php?url=' + p + 'SauceNAO</a>' +
				'<a class="de-menu-item de-imgmenu de-src-derpibooru" onclick="revSearch(this);return false;" src="' + p + 'Derpibooru</a>' + str;
		},
		...
		_clickMenu: function(el) {
			...
			switch (el.getAttribute('info')) {
				case 'spell-sel':
					...
					return;
				case 'spell-name':
					addSpell(6 /* #name */ , this.posterName.replace(/\)/g, '\\)'), false);
					return;
				case 'spell-trip':
					addSpell(7 /* #trip */ , this.posterTrip.replace(/\)/g, '\\)'), false);
					return;
				case 'spell-img':
				...
				case 'spell-notext':
					...
					return;
				case 'thr-exp':
					this.thr.load(parseInt(el.textContent, 10), false, null);
					return;
			}
		},
	...
	function Pview(parent, link, tNum, pNum) {
		...
		this.num = pNum;
		this.thr = parent.thr;
		Object.defineProperty(this, 'tNum', {
			value: tNum
		});
	...
	Pview.prototype = Object.create(Post.prototype, {
		...
		getTopParent: {
			value: function pvGetBoardParent() {
		...
		_handleMouseEvents: {
			value: function pvHandleMouseEvents(el, isOverEvent) {
		...
		_showPost: {
			value: function pvShowPost(post) {
		...
		_showPview: {
			value: function pvShowPview(el, id) {
	...
	Thread.prototype = {
		...
		_addPost: function(parent, el, i, prev) {
			...
			Object.defineProperty(post, 'wrap', {
				value: wrap
			});
	...
	function getImageBoard(checkDomains, checkOther) {
		var ibDomains = {
			'02ch.net': [{
				qPostRedir: {
					value: 'input[name="gb2"][value="thread"]'
				},
				ru: {
					value: true
				},
				timePattern: {
					value: 'yyyy+nn+dd++w++hh+ii+ss'
				}
			}],
			get '22chan.net' () {
				return this['ernstchan.com'];
			},
			'2chru.net': [{
				_2chru: {
					value: true
		...
		get '2-ch.so'() { return [ibEngines['section.posts']]; }, */
			...
			'2--ch.ru': [{
				...
				qPages: {
					value: 'table[border="1"] tr:first-of-type > td:first-of-type a'
				},
				qPostRedir: {
					value: null
				},
				qTable: {
					value: 'table:not(.postfiles)'
				},
				qThread: {
					value: '.threadz'
				},
				...
				css: {
					value: 'span[id$="_display"], #fastload { display: none !important; }'
				},
				docExt: {
					value: '.html'
				},
			...
			'420chan.org': [{ // Posting doesn't work (antispam protection)
				...
				css: {
					value: '#content > hr, .hidethread, .ignorebtn, .opqrbtn, .qrbtn, noscript { display: none !important; }\
			...
			'4chan.org': [{
				...
				qBan: {
					value: 'strong[style="color: red;"]'
				},
				qDelBut: {
					value: '.deleteform > input[type="submit"]'
				},
				qError: {
					value: '#errmsg'
				},
				...
				qPages: {
					value: '.pagelist > .pages:not(.cataloglink) > a:last-of-type'
				},
				qPostForm: {
					value: 'form[name="post"]'
				},
				qPostRedir: {
					value: null
				},
				qRef: {
					value: '.postInfo > .postNum'
				},
				qTable: {
					value: '.replyContainer'
				},
				qThumbImages: {
					value: '.fileThumb > img'
				},
				...
				css: {
					value: '.backlink, .extButton, hr.desktop, .navLinks, .postMenuBtn, #togglePostFormLink { display: none !important; }\
				...
				formButtons: {
					get: function() {
						return Object.create(this._formButtons, {
							tag: {
								value: ['**', '*', '__', '^H', 'spoiler', 'code', '', '', 'q']
							},
							bb: {
								value: [false, false, false, false, true, true, false, false, false]
							}
						});
					}
				},
			...
			'dobrochan.com': [{
				...
				qOmitted: {
					value: '.abbrev > span:first-of-type'
				},
				qPages: {
					value: '.pages > tbody > tr > td'
				},
				qPostRedir: {
					value: 'select[name="goto"]'
				},
				qTrunc: {
					value: '.abbrev > span:nth-last-child(2)'
				},
				getFileWrap: {
					value: function(el) {
						return el.parentNode;
				...
				css: {
					value: '.delete > img, .popup, .reply_, .search_google, .search_iqdb { display: none !important; }\
				...
				rLinkClick: {
					value: 'onclick="Highlight(event, this.getAttribute(\'de-num\'))"'
				},
				ru: {
					value: true
				},
				timePattern: {
					value: 'dd+m+?+?+?+?+?+yyyy++w++hh+ii-?s?s?'
				}
			}],
			get 'dobrochan.org' () {
			...
			'ernstchan.com': [{
				...
				css: {
					value: '.content > hr, .de-parea > hr { display: none !important }'
				}
			...
			'inach.org': [{
				qPostRedir: {
					value: 'input[name="fieldnoko"]'
				},
				css: {
					value: '#postform > table > tbody > tr:first-child { display: none !important; }'
				},
				isBB: {
					value: true
				}
			...
			'krautchan.net': [{
				...
				qPages: {
					value: 'table[border="1"] > tbody > tr > td > a:nth-last-child(2) + a'
				},
				qPostRedir: {
					value: 'input#forward_thread'
				},
				qRef: {
					value: '.postnumber'
				},
				...
				qThumbImages: {
					value: 'img[id^="thumbnail_"]'
				},
				qTrunc: {
					value: 'p[id^="post_truncated"]'
				},
				getFileWrap: {
					value: function(el) {
						return el.parentNode;
				...
				css: {
					value: 'img[id^="translate_button"], img[src$="button-expand.gif"], img[src$="button-close.gif"], body > center > hr, form > div:first-of-type > hr, h2, .sage { display: none !important; }\
				...
				rLinkClick: {
					value: 'onclick="highlightPost(this.textContent.substr(2)))"'
				},
				timePattern: {
					value: 'yyyy+nn+dd+hh+ii+ss+--?-?-?-?-?'
				}
			}],
			'lambdadelta.net': [{
				qHide: {
				...
				css: {
					value: '.content > hr { display: none !important }'
				}
			}, 'link[href$="phutaba.css"]'],
			'mlpg.co': [{
				getWrap: {
				...
				css: {
					value: '.image-hover, form > div[style="text-align: center;"], form > div[style="text-align: center;"] + hr { display: none !important; }'
				},
				formButtons: {
					get: function() {
						return Object.create(this._formButtons, {
							tag: {
								value: ['b', 'i', 'u', '-', 'spoiler', 'c', '', '', 'q']
							},
						});
					}
				},
			...
			'ponychan.net': [{
				...
				qPages: {
					value: 'table[border="0"] > tbody > tr > td:nth-child(2) > a:last-of-type'
				},
				css: {
					value: '#bodywrap3 > hr { display: none !important; }'
				}
			}, 'script[src*="kusaba"]'],
			'syn-ch.ru': [{
				css: {
					value: '.fa-sort, .image_id { display: none !important; }\
				...
				formButtons: {
					get: function() {
						return Object.create(this._formButtons, {
							tag: {
								value: ['b', 'i', 'u', 's', 'spoiler', 'code', 'sub', 'sup', 'q']
							},
						});
					}
				},
			...
			'touhouchan.org': [{
				...
				css: {
					value: 'span[id$="_display"], #bottom_lnks { display: none !important; }'
				},
				isBB: {
					value: true
				}
		...
		var ibEngines = {
			'section.posts': {
				...
				qBan: {
					value: 'font[color="#C12267"]'
				}, // FIXME
				qDForm: {
					value: '#posts-form, #posts_form, #delform'
				},
				qHide: {
					value: '.post-details ~ *'
				},
				qImgLink: {
					value: '.file-attr > .desktop'
				},
				qMsg: {
					value: '.post-message'
				},
				...
				cssEn: {
					value: '.thread-nav, .ABU-refmap, .media-expand-button, .postpanel, .rekl, .passcode-banner, .norm-reply, header > hr, .reflink::before { display: none !important; }\
				...
				formButtons: {
					get: function() {
						return Object.create(this._formButtons, {
							tag: {
								value: ['B', 'I', 'U', 'S', 'SPOILER', 'CODE', 'SUP', 'SUB', 'q']
							}
						});
					}
				},
			...
			'#ABU_css, #ShowLakeSettings': {
				...
				cssEn: {
					value: '#ABU_alert_wait, .ABU_refmap, #captcha_div + font, #CommentToolbar, .postpanel, #usrFlds + tbody > tr:first-child, body > center { display: none !important; }\
				.de-abtn { transition: none; }\
				#de-txt-panel { font-size: 16px !important; }\
			...
			'form[action*="futaba.php"]': {
				...
				qDForm: {
					value: 'form:not([enctype])'
				},
				qImgLink: {
					value: 'a[href$=".jpg"], a[href$=".png"], a[href$=".gif"]'
				},
				qOmitted: {
					value: 'font[color="#707070"]'
				},
				qPostForm: {
					value: 'form:nth-of-type(1)'
				},
				qPostRedir: {
					value: null
				},
				...
				qThumbImages: {
					value: 'a[href$=".jpg"] > img, a[href$=".png"] > img, a[href$=".gif"] > img'
				},
				getPageUrl: {
					value: function(b, p) {
				...
				cssEn: {
					value: '.de-cfg-body, .de-content { font-family: arial; }\
			...
			'form[name*="postcontrols"]': {
				...
				qDForm: {
					value: 'form[name="postcontrols"]'
				},
				qHide: {
					value: '.intro ~ *'
				},
				qImgLink: {
					value: 'p.fileinfo > a:first-of-type'
				},
				qMsg: {
					value: '.body'
				},
				...
				qPages: {
					value: '.pages > a:nth-last-of-type(2)'
				},
				qPostForm: {
					value: 'form[name="post"]'
				},
			...
			'script[src*="kusaba"]': {
				...
				qError: {
					value: 'h1, h2, div[style*="1.25em"]'
				},
				qPostRedir: {
					value: null
				},
				cssEn: {
					value: '.extrabtns, #newposts_get, .replymode, .ui-resizable-handle, blockquote + a { display: none !important; }\
				...
				rLinkClick: {
					value: 'onclick="highlight(this.textContent.substr(2), true)"'
				}
			},
			'link[href$="phutaba.css"]': {
				cSubj: {
				...
				qPages: {
					value: '.pagelist > li:nth-last-child(2)'
				},
				qPostRedir: {
					value: 'input[name="gb2"][value="thread"]'
				},
		...
		var ibBase = {
			...
			get qImgLink() {
				...
				Object.defineProperty(this, 'qImgLink', {
					value: val
				});
				return val;
			},
		...
		return {
			get ua() {
				...
				get canPlayWebm() {
					...
					Object.defineProperty(this, 'canPlayWebm', {
						value: val
					});
					return val;
				},
...
$(function() {
	$(".de-file-img").click(function() {
		//alert("catched");
		//clear_fake_rating();
		if (document.getElementsByClassName('de-file')[0].innerHTML.indexOf('rating') < 0) {
			document.getElementsByClassName('de-file')[0].innerHTML += pick_spoiler;
		}
	});
});

if (getCookie('hide_b') == 'true' ) {
	$("#bold").hide();
}

if (getCookie('hide_i') == 'true' ) {
	$("#italic").hide();
}

if (getCookie('hide_u') == 'true' ) {
	$("#u").hide();
}

if (getCookie('hide_s') == 'true' ) {
	$("#strike").hide();
}

if (getCookie('hide_spoiler') == 'true' ) {
	$("#spoiler").hide();
}

if (getCookie('hide_c') == 'true' ) {
	$("#code").hide();
}

if (getCookie('hide_sup') == 'true' ) {
	$("#sup").hide();
}

if (getCookie('hide_sub') == 'true' ) {
	$("#sub").hide();
}

if (getCookie('hide_atten') == 'true' ) {
	$("#attent").hide();
}

if (getCookie('hide_roll') == 'true' ) {
	$("#roll").hide();
}

if (getCookie('hide_quote') == 'true' ) {
	$("#quote").hide();
}